...
/* 
 * Draws pretty separator between two blocks if their colors differ
 * 
 */
void draw_separator(int x, int height, int width, uint32_t bg_colorpixel, uint32_t bg_colorpixel_next) {
	...
	TAILQ_FOREACH (block, &statusline_head, blocks) {
		...
		if (bg_colorpixel_next != bg_colorpixel) {
			...
			xcb_poly_line(xcb_connection, XCB_COORD_MODE_ORIGIN, statusline_pm,
						  statusline_ctx, 2,
						  (xcb_point_t[]) {{x - sep_offset, 3+2},
										   {x - sep_offset, 3+font.height - 2}});
		}
	}
}
