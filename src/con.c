...
bool con_should_gap(Con* con, bool border) {
	...
	while (con) { 
		...
		if (con_is_floating(con) || con->type == CT_DOCKAREA || (!border && con->parent && (con->parent->layout == L_STACKED || con->parent->layout == L_TABBED) && con_num_children(con->parent) > 1)) 
			return false;

		// if it has brothers - do GAP
		Con* child = NULL;
		TAILQ_FOREACH (child, &(con->nodes_head), nodes) {
			if (child != origCon && child->type == CT_CON) {
				return true;
			}
		}

		if (con->type == CT_WORKSPACE || con->type == CT_ROOT) 
			break;
