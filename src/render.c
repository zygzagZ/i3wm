...
void render_con(Con *con, bool render_fullscreen, bool already_inset) {
	...
	if (show_debug_borders) {
		const int debug_border_size = 2;
		rect.x += debug_border_size;
		rect.y += debug_border_size;
		rect.width -= 2 * debug_border_size;
		rect.height -= 2 * debug_border_size;
	}

	bool should_inset = (con_should_gap(con, false) && 
						 (con_is_leaf(con) ||
						  (children > 0 &&
	...
	if (con->layout == L_OUTPUT) {
		...
		TAILQ_FOREACH (child, &(con->nodes_head), nodes) {
			...
			/* stacked layout */
				...
				if (children < 2 && (child->border_style != BS_NORMAL || child->border_width < 4)) {
					child->deco_rect = (Rect) {0, 0, 0, 0};
				} else {
			...
			DLOG("child at (%d, %d) with (%d x %d)\n",
				 child->rect.x, child->rect.y, child->rect.width, child->rect.height);
			x_raise_con(child);
			render_con(child, false, should_inset || (already_inset && (child->type != CT_CON || !con_should_gap(con, false))));
			i++;
		}
